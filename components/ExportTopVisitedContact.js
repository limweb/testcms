...
export function ExportTopVisitedContact(props) {

    const { data = {}, className = '', date = {} } = props
    const fileName = `top_visited_contact__${date.startDate}__${date.endDate}`

    return (
        <ExcelFile filename={fileName} element={<button className={className}><i className="fas fa-file-excel mr-2"></i>Export</button>}>
            <ExcelSheet data={data} name="top_visited_contact">
